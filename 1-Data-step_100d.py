...
def process_days(dat):
    ...
    print(f"4-Save data to data/processed/100d/processed_{date}.feather")
    # Save data
    outdat = dat.reset_index(drop=True)
    outdat.to_feather(f"data/processed/100d/processed_{date}.feather")
...
files = glob.glob('data/processed/100d/*.feather')
files
list_ = []
for file in files:
